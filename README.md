...
* The observations follow a multivariate Gaussian distribution, as it assumes that each feature follows a one-dimensional normal distribution **X ∼ N(μ, Σ)**, with **some** correlation between the pairs.
...
Now we only have to replace this formula in the Bayes formula, which I previously mentioned, and see what is the maximum value of δk(x) = xT Σ−1μk − 1/2μTk Σ−1μk + log πk (LDA depends on the linear combination of the elements of x) given an observation classified as a _kth_ class.
